        ...
        <section className="bg-background py-16 md:py-24">
            <div className="section-shell">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {siteContent.homeGallery.map((item) => (
                        <div key={item.id} className="group relative aspect-[4/5] overflow-hidden rounded-2xl border border-black/10 dark:border-white/10">
                            ...
                            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/78 via-black/48 to-transparent p-5">
                                <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-primary-light">{item.category}</p>
                                <h4 className="text-xl font-serif font-bold leading-tight !text-white">{item.alt}</h4>
                            </div>
                        </div>
                    ))}
                </div>
